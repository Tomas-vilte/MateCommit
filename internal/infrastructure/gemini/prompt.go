...
const (
	...
	Commit: [type]: [message]
	Files:
	[list of modified files, each in a new line]
	...
	6. Include a newline character after each item in the file list.
	7. The ordinal must be correct (e.g., "First", "Second", "Third", etc.)
	`
	...
	Commit: [tipo]: [mensaje]
	Archivos:
	[lista de archivos modificados, cada uno en una nueva línea]
	...
	6. Incluí un salto de línea después de cada elemento en la lista de archivos.
	7. El ordinal tiene que ser correcto (e.g., "Primera", "Segunda", "Tercera", etc.)`
)
