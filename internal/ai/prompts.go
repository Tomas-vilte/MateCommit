...
const (
  ...
  2. **New vs. Existing (IMPORTANT):** Each file in "Main files modified" is tagged with its real git status. A file tagged [added] is genuinely new. A file tagged [modified] already existed — do NOT describe changes inside it as "added a new feature" or "implemented X" unless the diff clearly shows a brand-new exported function/type/symbol that didn't exist before. For changes to existing code, say "extended", "changed", or "fixed" instead of "added". When the available labels include a distinction like "feature" (new) vs "enhancement" (extends existing), pick the one that matches what the diff actually shows.
  3. **Tone:** Professional, direct, technical. Use first person ("I implemented", "I added"). Sound like an engineer describing their own work, not a template — vary sentence structure, don't tack a generic justification onto every bullet. Avoid AI/marketing buzzwords (leverage, robust, seamless, streamline, enhance, empower, cutting-edge).
  4. **No Formulaic Openers:** Never start the body with a stock phrase like "This PR enhances/improves the quality of...", "This PR introduces changes to...", or "In this PR, I focused on...". Open with the concrete first change instead.
  ...
  3. Labels: Pick the MOST SPECIFIC match from the actual "Available Labels" list provided above (with their descriptions) — don't default to a generic label when a more precise one fits. Never invent a label that isn't in that list.
  ...
  2. **Nuevo vs. existente (IMPORTANTE):** Cada archivo en "Main files modified" tiene su status real de git. Un archivo con [added] es genuinamente nuevo. Un archivo con [modified] ya existía antes — NO describas cambios ahí como "agregué una nueva feature" o "implementé X" salvo que el diff muestre claramente una función/tipo/símbolo exportado totalmente nuevo que no existía antes. Para cambios en código existente, usá "extendí", "cambié" o "arreglé" en vez de "agregué". Si entre los labels disponibles hay una distinción tipo "feature" (nuevo) vs "enhancement" (extiende algo existente), elegí el que corresponda según lo que realmente muestra el diff.
  3. **Tono:** Profesional, cercano y directo. Usa primera persona ("Implementé", "Agregué", "Corregí"). Evita el lenguaje robótico ("Se ha realizado"). Sonás como un dev contando su propio trabajo, no como un molde — variá la estructura de las oraciones, no le pegues una justificación genérica a cada bullet. Evitá muletillas de marketing/IA (robusto, optimizar como palabra comodín, mejorar la experiencia, potenciar, de vanguardia).
  4. **Cero aperturas de molde:** Nunca arranques el body con una frase hecha tipo "Este PR mejora la calidad de...", "Este PR introduce cambios en..." o "En este PR me enfoqué en...". Arrancá directo con el primer cambio concreto.
  5. **Formato Markdown (ESTRICTO):** Escribí el campo "body" como un documento Markdown real de varias líneas — apretá enter entre líneas como en un documento de verdad, NO escribas la secuencia de escape de dos caracteres (barra invertida seguida de la letra n) como texto literal en ningún lugar del body. Cada "## Encabezado" va en su propia línea seguido de una línea en blanco. Cada checkbox ("- [ ] ...") va en su propia línea. NUNCA mezcles varias secciones o checkboxes en un solo párrafo de texto corrido, y NUNCA generes esa secuencia de escape en lugar de un salto de línea real.
  # Instrucciones
  ...
  3. Etiquetas: Elegí la MÁS ESPECÍFICA de la lista real de "Available Labels" de arriba (con sus descripciones) — no uses un label genérico si hay uno más preciso. Nunca inventes un label que no esté en esa lista.
